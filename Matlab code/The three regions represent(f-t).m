...
% 🌟 實驗參數設定區（自動連動圖片與運算邏輯）
% =======================================================
delayValue = 100;                     % 【輸入參數】延遲時間 (Delay，請直接輸入純數字)
velocity   = 46.25 / delayValue;         % 【自動換算】剝離速度 (cm/s)

% 👇 --- 自訂波峰抓取條件 --- 👇
time_start      = 0;     % 【條件 0：起始時間】只分析此時間 (秒) 之後的數據
time_end        = 60;   % 【條件 0：結束時間】只分析此時間 (秒) 之前的數據 (若不限制可填入 Inf)
min_force_limit = 2.7;   % 【條件 1：範圍以上】只抓力量大於此數值 (N) 的數據
min_drop_value  = 0.2;   % 【條件 2：垂直落差】波峰到下一個谷底，力量至少要掉落多少 (N)
% =======================================================
...
% 標題自動套用換算速度與Delay
title_str1 = sprintf('%.4f mm/s (Delay %d): Complete Raw Waveform', velocity, delayValue); 
...
% 標題自動套用換算速度與Delay
title_str2 = sprintf('%.4f mm/s (Delay %d): Analyzed Force vs Time', velocity, delayValue); 
